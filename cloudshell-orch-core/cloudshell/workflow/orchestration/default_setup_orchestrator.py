...
class DefaultSetupWorkflow(object):
    ...
    def default_provisioning(self, sandbox, components):
        ...
        self._deploy_result = SetupCommon.deploy_apps_in_reservation(api=api,
                                                                     reservation_details=reservation_details,
                                                                     reservation_id=sandbox.id,
                                                                     logger=sandbox.logger)

        SetupCommon.validate_all_apps_deployed(deploy_results=self._deploy_result,
                                               logger=sandbox.logger)

        sandbox.components.add_deployed_apps_info(sandbox=sandbox,
                                                  deployment_results=self._deploy_result)
